...
class pedir_multi_titulos(models.TransientModel):
    _name = 'pedir_multi_titulos'
    _description = 'Pedir Multiples Titulos'

    linea_ids = fields.Many2many(
        'pedido_multi_titulo.linea', string='Títulos')
    ...
    def default_get(self, cr, uid, fields, context=None):
        res = super(pedir_multi_titulos, self).default_get(
            cr, uid, fields, context=context)

        linea_ids = []
        for product_id in context.get('active_ids', []):
            vals = {'product_id': product_id, 'cantidad': 1}
            linea_id = pooler.get_pool(cr.dbname).get(
                'pedido_multi_titulo.linea').create(cr, uid, vals, context=context)
    ...
    def a_nuevo_pedido(self, cr, uid, ids, context=None):
        ...
        if not pricelist_id:
            any_pricelist_id = self.pool.get(
        ...
        partner_order_id = partner.address.id
        ...
        for linea in self.browse(cr, uid, ids)[0].linea_ids:
            self.agregar_titulo_a_pedido(
                cr, uid, order_id, linea, context=context)
    ...
    def agregar_titulo_a_pedido(self, cr, uid, order_id, linea, context=None):
        ...
        # name_get retorna una lista con pares cuyo primer valor es el id y el
        # segundo el nombre, por eso el acceso [0][1]
        product_obj = self.pool.get('product.product')
        name = product_obj.name_get(
            cr, uid, [product.id], context=context)[0][1]

        product_uom_qty = linea.cantidad

        uom_ids = pooler.get_pool(cr.dbname).get('product.uom').search(
            cr, uid, [('name', '=', 'PCE')], context=context)
        if not uom_ids:
            uom_id = pooler.get_pool(cr.dbname).get(
                'product.uom').search(cr, uid, [], context=context)[0]
        else:
            uom_id = uom_ids[0]
        ...
        new_ids = pooler.get_pool(cr.dbname).get(
            'sale.order').write(cr, uid, order_id, new_values_order)
...
class pedido_multi_titulo_linea(models.Model):
