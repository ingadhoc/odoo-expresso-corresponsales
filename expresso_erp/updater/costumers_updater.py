...
class Customers_updater(Actualizador_Generico):

    def __init__(self, pooler):
        Actualizador_Generico.__init__(
            self, pooler, configuracion_actualizacion.url_clientes)
    ...
    def actualizar_clientes(self, cr, uid, context=None):
        ...
        info_corresponsal_ids = info_corresponsal_obj.search(
            cr, uid, [], context=context)

        clientes_procesados = 0

        _logger.info(
            'Hay %s Clientes para procesar', len(info_corresponsal_ids))

        for info_corresponsal_id in info_corresponsal_ids:
            info_corresponsal = info_corresponsal_obj.browse(
                cr, uid, info_corresponsal_id, context=context)
            ...
            if hasattr(cliente_expresso, 'error'):
                _logger.error('Error procesando corresponsal %s. Error: %s',
                              info_corresponsal.corresponsal, cliente_expresso.error)
                continue
    ...
    def process_cliente(self, cr, uid, cliente, info_corresponsal=None, context=None):
        ...
        # Remplazo los retorno de linea por puntos ya que los campos donde se
        # van a guardar no son texto sino cadenas de caracteres.
        if address_street:
        ...
        if cliente.pais:
            ...
            country_ids = self.pooler.get_pool(cr.dbname).get('res.country').search(
                cr, uid, [('name', '=', country_name)], context=context)
            if country_ids:
        ...
        new_values_for_partner = {
            'id_remoto': id_remoto, 'name': name, 'active': active, 'vat': vat, 'comment': comment}
        if info_corresponsal:
            new_values_for_partner[
                'info_corresponsal_id'] = info_corresponsal.id

        stored_partner_ids = self.get_ids_from_id_remoto(
            cr, uid, 'res.partner', id_remoto, context=None)
        ...
        if not stored_partner_ids:
            stored_partner_ids = [self.pooler.get_pool(cr.dbname).get(
                'res.partner').create(cr, uid, new_values_for_partner)]
        else:
            self.pooler.get_pool(cr.dbname).get('res.partner').write(
                cr, uid, stored_partner_ids, new_values_for_partner)
        ...
        for stored_partner_id in stored_partner_ids:
            ...
            if not stored_address_ids:
                self.pooler.get_pool(cr.dbname).get('res.partner.address').create(
                    cr, uid, new_values_for_address, context=context)
            else:
                self.pooler.get_pool(cr.dbname).get('res.partner.address').write(
                    cr, uid, stored_address_ids, new_values_for_address, context=context)
        ...
        stored_user_ids = self.get_ids_from_id_remoto(
            cr, uid, 'res.users', id_remoto, context=None)
        # Si no hay ningún res.user con el id_remoto especificado, puede ser
        # que este alamcenado con el campo active igual a False.
        ...
        if not stored_user_ids:
            stored_user_ids = [
                usr_obj.create(cr, uid, new_values_for_user, context=context)]
        else:
            usr_obj.write(
                cr, uid, stored_user_ids, new_values_for_user, context=context)
